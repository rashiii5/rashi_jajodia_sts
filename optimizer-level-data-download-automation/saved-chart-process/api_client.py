...
def get_site_overview(site_id: int) -> dict:
    """Fetch /site/{id}/overview — returns the raw overview dict."""
    data = _session.get(f"/site/{site_id}/overview")
    return data.get("overview", {})


def get_site_energy_yesterday(site_id: int, date_str: str) -> float:
    """
    Fetch /site/{id}/energy for the previous day.
    date_str is the run date (YYYY-MM-DD); we request date-1.
    Returns energy in kWh (converted from Wh).
    """
    from datetime import datetime, timedelta
    prev = (datetime.strptime(date_str, "%Y-%m-%d") - timedelta(days=1)).strftime("%Y-%m-%d")
    data = _session.get(
        f"/site/{site_id}/energy",
        startTime=f"{prev} 00:00:00",
        endTime=f"{prev} 23:59:59",
        timeUnit="DAY",
    )
    values = data.get("energy", {}).get("values", [])
    for v in values:
        if v.get("value") is not None:
            return round(v["value"] / 1000, 4)
    return 0.0


def get_equipment_telemetry(site_id: int, serial: str, date_str: str) -> dict:
    """
    Fetch /equipment/{siteId}/{serial}/data for the previous day.
    Returns the latest telemetry entry dict, or {} if none.
    """
    from datetime import datetime, timedelta
    prev = (datetime.strptime(date_str, "%Y-%m-%d") - timedelta(days=1)).strftime("%Y-%m-%d")
    data = _session.get(
        f"/equipment/{site_id}/{serial}/data",
        startTime=f"{prev} 00:00:00",
        endTime=f"{prev} 23:59:59",
    )
    entries = data.get("data", {}).get("telemetries", [])
    # Return the last entry with a timestamp
    for entry in reversed(entries):
        if entry.get("date") or entry.get("dateTime") or entry.get("time"):
            return entry
